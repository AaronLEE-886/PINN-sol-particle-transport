...
def data_loss(model, s_data, T_data) -> torch.Tensor:
    """Return mean-squared data misfit on observed temperature samples."""
    T_pred = model(s_data)
    return torch.mean((T_pred - T_data) ** 2)


def combined_loss(model, s_colloc, config: SOLConfig, loss_weights=None,
                  source_fn=None, s_data=None, T_data=None) -> dict:
    ...
    if s_data is not None and T_data is not None:
        L_data = data_loss(model, s_data, T_data)
        losses["data"] = L_data
